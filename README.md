...
> ### V2.2 - V2.5 (Текущее)
> * ### Нововведения и улучшения:
  > 1) Исправлена недоработка, при которой основные глобальные переменные были привязаны для всех пользователей, из-за которых происходили непонимания в выполнении некоторых действий.
  > 2) Добавлено отслеживание активности Бота, теперь боле понятно, что бот в процессе ответа, а не простаивает.
  ...
  > 3) Завершение шагов к выходу на подключение сторонних групп.
  > 4) Визуальная переработка отображения основных данных консоли.
  > 5) Обновлены таймеры "Авто-отправки дз" и "Изменения типа недели"
  > 6) Завершение обновления базы данных.
  ...
  > 7) Проведено 90% тестов на выявление ошибок. 
  > 8) Проведена дополнительная оптимизация кода.
  >      - Различные улучшения;
  ...
  > 9) Добавлены/исправлены функции (ч2), которые могут ускорить взаимодействие.  
  ...
  > 10) Добавлены новые [команды](#актуальные-команды), обновлен список [ошибок](#список-ошибок)
  ...
  > 11) _Добавлена возможность добавить замены пар или полностью поменять все пары для Дз._
  > 12) Теперь кнопка под сообщением **`"🖼В виде картинки"`** выводит именно те данные под которыми она находилась _(последние 5 сообщений с данной кнопкой)_, а не последнее под чем она появлялась.
  > 13) Убран моментальный бан за спам => Заменено на временный мут на 10мин.
  > 14) Добавлено подтверждение перед удалением звонков.
  > 15) Все необходимые папки для работы бота вынесены в начальную папку.
  > 16) Первые попытки выхода на внешние сервера:
